...
def get_sbert_embedding(label):
    if label not in _emb_cache:
        ...
        _emb_cache[label] = embedding.detach().cpu().numpy()
...
def get_batch_embeddings(labels):
    ...
    if uncached:
        new_embs = sbert_model.encode(uncached, convert_to_tensor=True, device=DEVICE)
        for label, emb in zip(uncached, new_embs):
            _emb_cache[label] = emb.detach().cpu().numpy()
