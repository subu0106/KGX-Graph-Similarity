#!/usr/bin/env python3
"""
Run KEA similarity on MRPC dataset (paraphrase pairs with KGs).
...
from Methods import calculate_similarity


def process_mrpc_dataset(input_file, output_file):
    """Process MRPC dataset and calculate KEA similarity between kg_1 and kg_2."""

    results = []
    fieldnames = ['id', 'paragraph_1', 'paragraph_2', 'kg_1', 'kg_2', 'kea_similarity']
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as outfile:
        ...
        for idx, row in enumerate(rows, 1):
            ...
            if kg1 and kg2:
                try:
                    kea_sim = calculate_similarity(kg1, kg2)
                except Exception as e:
                    print(f"  ERROR in KEA: {e}")
                    kea_sim = 0.0
            else:
                kea_sim = 0.0
    ...
    similarities = [r['kea_similarity'] for r in results if r['kea_similarity'] > 0]
...
def generate_summary(results, output_dir):
    ...
    valid_sims = [r['kea_similarity'] for r in results if r['kea_similarity'] > 0]
    ...
    summary_file = os.path.join(output_dir, "mrpc_kea_summary.txt")

    with open(summary_file, 'w') as f:
        f.write("MRPC KEA SUMMARY STATISTICS\n\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='KEA Similarity for mrpc Dataset')
    parser.add_argument('--input',  type=str,
                        default=os.path.join(script_dir, 'mrpc_400_KGs.csv'),
                        help='Input CSV file')
    parser.add_argument('--output', type=str,
                        default=os.path.join(script_dir, 'mrpc_kea_results.csv'),
